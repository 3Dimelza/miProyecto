...
<br>
		...
		if(count($users)>0){
			...
			foreach($users as $user){
				...
				<td><?php echo $user->fechaActualizacion; ?></td>		
				<td style="width:30px;"><a href="index.php?view=edituser&id=<?php echo $user->id;?>" class="btn btn-warning btn-xs">Editar</a></td>
				
				<td style="width:30px;">
            	<a href="index.php?view=1deluser&id=<?php echo $user->id; ?>" class="btn btn-danger btn-xs" onclick="return confirm('¿Está seguro de que desea eliminar este usuario?');">Eliminar</a>
        		</td>
				</tr>
				<?php
